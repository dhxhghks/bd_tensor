...
# tag::dl_agent_predict[]
    def predict(self, game_state):
        ...
        print(f'predict.py encoded_state ->{encoded_state.dtype}')
        print(f'predict.py encoded_state ->{encoded_state.shape}')
        # for m in encoded_state:
        #     print(f'predict.py predict encoded_state ->{m}')
        input_tensor = np.array([encoded_state])
        # print(f'predict.py input_tensor type ->{type(input_tensor)}')
        print(f'predict.py input_tensor dtype ->{input_tensor.dtype}')
        print(f'predict.py input_tensor shape ->{input_tensor.shape}')        
        model_output = self.model.predict(input_tensor)
        print(f'predict.py model_output dtype ->{model_output.dtype}')
        print(f'predict.py model_output shape ->{model_output.shape}')        
